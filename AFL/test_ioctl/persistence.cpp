...
ULONG_PTR COV_ADDR = 0;
ULONG_PTR KCOV_ADDR = 0;
ULONG_PTR MDL_ADDR = 0;
ULONG_PTR shared_mem;
HANDLE pipe;
...
SIZE_T get_helper_value(LPCSTR symbol) {
	SIZE_T v = get_driver_address("helper.sys", symbol);
	SIZE_T v2 = 0;
...
void setup_shmem() {
	...
	shared_mem = (ULONG_PTR)MapViewOfFile(map_file, // handle to map object
...
void PRE() {
	...
	memcpy((void*)COV_ADDR, buf, sizeof(buf));
}
...
void POST() {
	...
	memcpy((void *)shared_mem, (void *)COV_ADDR, MAP_SIZE);
	MEM_BARRIER();
...
void INIT() {
	...
	if (_tgetenv(SHM_ENV_VAR)) {
		...
		KCOV_ADDR = (ULONG_PTR)HELPER_GetModuleSectionAddress(target);
		LOG(_T("Kernel coverage map @ 0x%p\n"), KCOV_ADDR);
		if (!KCOV_ADDR) {
		...
		HELPER_MapMemory(KCOV_ADDR, MAP_SIZE + PAGE_SIZE, &COV_ADDR, &MDL_ADDR);
		LOG(_T("User coverage map @ 0x%p\n"), COV_ADDR);
		LOG(_T("MDL @ 0x%p\n"), MDL_ADDR);
