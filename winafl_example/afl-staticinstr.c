...
#define AFL_VARIABLE_BEHAVIOR_TRACES_BASE_DIR TEXT("C:\\test\\traces")
#define AFL_VARIABLE_BEHAVIOR_ITERATIONS 20
#define MAX_STRING_SIZE 64

#pragma pack(push, 1)
typedef struct {
    CHAR __afl_area[MAP_SIZE];
    UINT_PTR __afl_prev_loc;
    UINT __tls_index;
    UINT __tls_slot_offset;
...
#define kMaximumInstrumentedModules 1
...

{
    ...
    Status = EnumProcessModulesEx(GetCurrentProcess(), Modules, sizeof(Modules), &SizeNeeded, LIST_MODULES_ALL);
    ...
    for(i = 0; i < SizeNeeded / sizeof(Modules[0]); ++i) {
        ...
        for(j = 0; j < NtHeaders->FileHeader.NumberOfSections; ++j) {
            ...
            g_static_coverage_data[g_ninstrumented_modules] = (STATIC_COVERAGE_DATA*)(
                (UINT_PTR)(Sections[j].VirtualAddress) + (UINT_PTR)Base
            );
            memset(g_static_coverage_data[g_ninstrumented_modules]->__afl_area, 0, MAP_SIZE + 8);
...

{
    ...
    //
    if (g_current_iterations == 0) {
        g_current_iterations+=1;
        return TRUE;
    }
    memcpy(g_static_coverage_data[0]->__afl_area_ptr, g_static_coverage_data[0]->__afl_area, MAP_SIZE);
...

    {
        ...
        if(g_current_iterations > 1 && !IsDebuggerPresent()) {
            ...
            _stprintf_s(
                ...
                AFL_VARIABLE_BEHAVIOR_TRACES_BASE_DIR L"\\%u.bin", g_current_iterations - 1
            );
    ...
    if(g_noinstrumentation == FALSE) {
        ...
        for(i = 0; i < g_ninstrumented_modules; ++i) {
            ...
            if(CurrentCoverageData->__tls_slot_offset != 0) {
                ...
                PUINT Base = (PUINT)(__readgsqword(0x2C) + (4 * CurrentCoverageData->__tls_index));
                PerThreadPrevLoc = (PUINT)(*Base + CurrentCoverageData->__tls_slot_offset);
            } else {
                PerThreadPrevLoc = (PUINT)(&CurrentCoverageData->__afl_prev_loc);
            }

            *PerThreadPrevLoc = 0;
        }

        memset(g_static_coverage_data[0]->__afl_area, 0, MAP_SIZE + 8);
